...
class Client():
    def __init__(self, data_dir):
        ...
        self.fixed_noise.data.copy_(fixed_noise_).to(self.device)
        ...
        train_set = datasets.ImageFolder(self.data_dir, transform=transform)
    ...
    def local_training(self):
        ...
        for epoch in range(self.local_epochs):
            for i, (img, label) in enumerate(self.train_loader):
                ...
                with torch.no_grad():
                    ...
                    self.s_label.resize_(self.batch_size, 1 ).fill_(self.real_label)
                    self.c_label.resize_(self.batch_size ).copy_(label)
                s_output, c_output = self.discriminator(img)
                s_output = s_output.to(self.device)
                c_output = c_output.to(self.device)
                img = img.to(self.device)
                label = label.to(self.device)
        ...
        torch.save(self.generator.state_dict(), '%s/netG_epoch_%d.pth' % (os.path.join('COVID', '0_saved_model'), epoch))
        self.test_image(self.generator)
        # self.saved_training(self.losses_d,self.losses_g)
    ...
    def test_image(self,model):
        PATH = 'DIS+GEN/netG_epoch_500.pth'
